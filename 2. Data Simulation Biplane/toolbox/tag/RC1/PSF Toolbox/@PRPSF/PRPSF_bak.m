...
%                 Edge=[mean(Mpsfo(4,:)),mean(Mpsfo(R1-3,:)),mean(Mpsfo(:,4)),mean(Mpsfo(:,R1-3))];   %Edited by FX
%                 Edge=[mean(Mpsfo(4,4:R1-3)),mean(Mpsfo(R1-3,4:R1-3)),mean(Mpsfo(4:R1-3,4)),mean(Mpsfo(4:R1-3,R1-3))];   %Edited by FX
...
%                 
                bg=max(Edge);
%                bg = sum(sum((1-circleF) .* Mpsfo)) / sum(sum(1-circleF));
%                bg = max(max((1-circleF) .* Mpsfo))
            %    bg
                Fig2=(Mpsfo-bg);
                Fig2 = Fig2 .* circleF; %??? whether need circleF
                Fig2(Fig2<=0)=0;
                tmp=zeros(R,R);
