classdef PSF_zernike_cryoFM < handle
    ...
    properties (SetAccess = public, GetAccess = public) %FX change, old SetAccess = private
    ...
    methods
        function obj=PSF_zernike_cryoFM(PRstruct)
            %probj=prpbj;
...
%             obj.Z = zk;
        ...
        function genPupil(obj)
            ...
            %N=numel(ceffp);
            for k = 1 : n
                pupil_phase = pupil_phase + obj.Z.ZM(:, :, k) .* obj.phase_coefficients(k);
            end

            obj.mag_coefficients = zeros(n);
            obj.mag_coefficients(1) = 1;
            for k = 1 : n
                pupil_mag = pupil_mag + obj.Z.ZM(:, :, k) .* obj.mag_coefficients(k);
            end
            obj.Pupil.phase=exp(pupil_phase.*1i);
            obj.Pupil.mag=pupil_mag;
            obj.PRstruct.Pupil.phase=exp(pupil_phase.*1i);
            obj.PRstruct.Pupil.mag=pupil_mag;

%             if isreal(pupil_phase)
%                 pupil_phase=exp(pupil_phase.*1i);
%             end
...
% %             PRphase=tmp.*mag;
%             
%             obj.Pupil.phase =  obj.PRstruct.Pupil.phase;
%             obj.Pupil.mag = obj.PRstruct.Pupil.mag;
%             obj.PRstruct.Pupil.phase =  obj.PRstruct.Pupil.phase;
%             obj.PRstruct.Pupil.mag = obj.PRstruct.Pupil.mag;
            
        end
        
        function setPupil(obj)
            obj.Pupil.phase = obj.PRstruct.Pupil.phase;
%             obj.Pupil.mag = obj.PRstruct.Pupil.mag;
...
%             obj.PRstruct.Pupil.mag=obj.PRstruct.Pupil.mag;
        ...
        function padPupil(obj)
            pad_size = (obj.PSFsize - size(obj.PRstruct.Pupil.phase,1)) / 2;
            obj.Pupil.phase = padarray(obj.PRstruct.Pupil.phase, [pad_size pad_size]);
            obj.Pupil.mag = padarray(obj.PRstruct.Pupil.mag, [pad_size pad_size]);
%             obj.Pupil.phase = extend(obj.PRstruct.Pupil.phase, obj.PSFsize);
%             obj.Pupil.mag = extend(obj.PRstruct.Pupil.mag, obj.PSFsize); 
        end
        ...
        function genPSF_2(obj)
            % genPSF_2 - generate PSFs from the given pupil function.
            %   phase are complex number
            %   The PSFs are directly calculated from the Fourier transform
            %   of pupil functions modified by shift phase in x, y and
            %   defocus phase in z. The out put is 'PSFs'
            N=numel(obj.Xpos);
            ...
            psfs=zeros(Ri,Ri,N);
            for ii=1:N
                shiftphase=-obj.k_r.*cos(obj.Phi).*obj.Xpos(ii).*obj.Pixelsize-obj.k_r.*sin(obj.Phi).*obj.Ypos(ii).*obj.Pixelsize;  % XY position
                shiftphaseE=exp(-1i.*2.*pi.*shiftphase);
                defocusphaseE=exp(2.*pi.*1i.*obj.Zpos(ii).*obj.k_z);
...
%                 tmp = imfilter(obj.Pupil.phase,A);
%                 [U, S, V] = svd(obj.Pupil.phase);
%                 index = S>S(6,6);
%                 S2 = S.*index;
%                 test = U*S2*V';

%                 pupil_complex=obj.Pupil.mag.*tmp.*shiftphaseE.*defocusphaseE;
        ...
        function genIMMPSF(obj)
            ...
            %dipshow(angle(IMMphase))
            test = 2*pi/obj.PRstruct.Lambda.*deltaH.*obj.NA_constrain;
          %  phase_test =  obj.Pupil.phase + 2*pi/obj.PRstruct.Lambda.*deltaH.*obj.NA_constrain;
            % relative z position of the emitters inside the sample medium
            
            % IMMpupil --> the pupil of index mismatch, edited by FX
%             IMMpupil = obj.Pupil.mag.*exp(obj.Pupil.phase.*1i).*IMMphase;
%             dipshow(angle(IMMpupil));
