...
%R=obj.PSF_size_forMLE;
%z=[obj.Zstart:obj.Zstep:obj.Zend];
z = obj.Zpos;
N=obj.DatadimZ;
[val,ind]=min(abs(z));
realsize0=floor(obj.OTFratioSize/2); %60/2=30
realsize1=ceil(obj.OTFratioSize/2);
R=size(obj.Mpsf_extend(:,:,ind),1); %measured psf
%R1=size(obj.Mpsf_extend(:,:,ind),1); %measured psf
%R2=size(obj.PSFstruct.PRpsf(:,:,ind),1); %retrieved psf

starty=-realsize0+R/2+1;endy=realsize1+R/2;
startx=-realsize0+R/2+1;endx=realsize1+R/2;

%starty1=-realsize0+R1/2+1;endy1=realsize1+R1/2;
%startx1=-realsize0+R1/2+1;endx1=realsize1+R1/2;

%starty2=-realsize0+R2/2+1;endy2=realsize1+R2/2;
%startx2=-realsize0+R2/2+1;endx2=realsize1+R2/2;
